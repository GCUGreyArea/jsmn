...
int main(int argc, char ** argv) {
    ...
    if(count < 0) {
        std::cerr << "Failed to aquire parsed tokens from: " << argv[1] << std::endl;
        return -1;
    }
        ...
        if(search != nullptr) {
            ...
            if(path != nullptr) {
                ...
                if(val != nullptr) {
                    ...
                    std::cout << std::endl << "Path value for " << argv[2] << "  : " << str << std::endl;
                }
				else {
					std::cerr << "Failed to find path: " << argv[2] << std::endl;
					exit -1;
				}
            }
        }
