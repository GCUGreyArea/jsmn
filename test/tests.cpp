...
int test_array_nomem(void) {
  ...
  m_js = "  [ 1, true, [123, \"hello\"]]";

  for (i = 0; i < 6; i++) {
    jsmn_init(&p);
    memset(toksmall, 0, sizeof(toksmall));
    memset(toklarge, 0, sizeof(toklarge));
    r = jsmn_parse(&p, m_js, strlen(m_js), toksmall, i);
    check(r == JSMN_ERROR_NOMEM);

    memcpy(toklarge, toksmall, sizeof(toksmall));

    r = jsmn_parse(&p, m_js, strlen(m_js), toklarge, 10);
    check(r >= 0);
    check(tokeq(m_js, toklarge, 4, JSMN_ARRAY, -1, -1, 3, JSMN_PRIMITIVE, "1",
                JSMN_PRIMITIVE, "true", JSMN_ARRAY, -1, -1, 2, JSMN_PRIMITIVE,
                "123", JSMN_STRING, "hello", 0));
  }
  return 0;
...
int test_unquoted_keys(void) {
  ...
  r = jsmn_parse(&p, m_js, strlen(m_js), tok, 10);
  check(r >= 0);
  check(tokeq(m_js, tok, 4, JSMN_PRIMITIVE, "key1", JSMN_STRING, "value", 0,
...
int test_issue_22(void) {
  ...
  jsmn_init(&p);
  r = jsmn_parse(&p, m_js, strlen(m_js), tokens, 128);
  check(r >= 0);
  return 0;
}
...
int test_issue_27(void) {
  ...
  check(parse(m_js, JSMN_ERROR_PART, 8));
  return 0;
}
...
int test_input_length(void) {
  ...
  jsmn_init(&p);
  r = jsmn_parse(&p, m_js, 8, tokens, 10);
  check(r == 3);
  check(tokeq(m_js, tokens, 3, JSMN_OBJECT, -1, -1, 1, JSMN_STRING, "a", 1,
              JSMN_PRIMITIVE, "0"));
  return 0;
}
...
int test_count(void) {
  ...
  m_js = "{}";
  jsmn_init(&p);
  check(jsmn_parse(&p, m_js, strlen(m_js), NULL, 0) == 1);
  ...
  m_js = "[[]]";
  jsmn_init(&p);
  check(jsmn_parse(&p, m_js, strlen(m_js), NULL, 0) == 2);

  m_js = "[[], []]";
  jsmn_init(&p);
  check(jsmn_parse(&p, m_js, strlen(m_js), NULL, 0) == 3);
  ...
  m_js = "[[], [[]], [[], []]]";
  jsmn_init(&p);
  check(jsmn_parse(&p, m_js, strlen(m_js), NULL, 0) == 7);

  m_js = "[\"a\", [[], []]]";
  jsmn_init(&p);
  check(jsmn_parse(&p, m_js, strlen(m_js), NULL, 0) == 5);
  ...
  m_js = "[1, 2, 3]";
  jsmn_init(&p);
  check(jsmn_parse(&p, m_js, strlen(m_js), NULL, 0) == 4);
...
int test_nonstrict(void) {
  ...
  const char *m_js;
  m_js = "a: 0garbage";
  check(parse(m_js, 2, 2, JSMN_PRIMITIVE, "a", JSMN_PRIMITIVE, "0garbage"));

  m_js = "Day : 26\nMonth : Sep\n\nYear: 12";
  check(parse(m_js, 6, 6, JSMN_PRIMITIVE, "Day", JSMN_PRIMITIVE, "26",
              JSMN_PRIMITIVE, "Month", JSMN_PRIMITIVE, "Sep", JSMN_PRIMITIVE,
              "Year", JSMN_PRIMITIVE, "12"));

  /* nested {s don't cause a parse error. */
  m_js = "\"key {1\": 1234";
  check(parse(m_js, 2, 2, JSMN_STRING, "key {1", 1, JSMN_PRIMITIVE, "1234"));
...
int test_unmatched_brackets(void) {
  const char *m_js;
  m_js = "\"key 1\": 1234}";
  check(parse(m_js, JSMN_ERROR_INVAL, 2));
  m_js = "{\"key 1\": 1234";
  check(parse(m_js, JSMN_ERROR_PART, 3));
  m_js = "{\"key 1\": 1234}}";
  check(parse(m_js, JSMN_ERROR_INVAL, 3));
  m_js = "\"key 1\"}: 1234";
  check(parse(m_js, JSMN_ERROR_INVAL, 3));
  m_js = "{\"key {1\": 1234}";
  check(parse(m_js, 3, 3, JSMN_OBJECT, 0, 16, 1, JSMN_STRING, "key {1", 1,
              JSMN_PRIMITIVE, "1234"));
  m_js = "{\"key 1\":{\"key 2\": 1234}";
  check(parse(m_js, JSMN_ERROR_PART, 5));
  return 0;
}
...
int test_object_key(void) {
  ...
  m_js = "{\"key\": 1}";
  check(parse(m_js, 3, 3, JSMN_OBJECT, 0, 10, 1, JSMN_STRING, "key", 1,
              JSMN_PRIMITIVE, "1"));
#ifdef JSMN_STRICT
  m_js = "{true: 1}";
  check(parse(m_js, JSMN_ERROR_INVAL, 3));
  ...
  check(parse(m_js, JSMN_ERROR_INVAL, 3));
  m_js = "{{\"key\": 1}: 2}";
  check(parse(m_js, JSMN_ERROR_INVAL, 5));
  m_js = "{[1,2]: 2}";
  check(parse(m_js, JSMN_ERROR_INVAL, 5));
#endif
  return 0;
